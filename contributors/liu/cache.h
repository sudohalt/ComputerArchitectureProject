...
#define DEFAULT_BIPCTRSIZE 32
#define DEFAULT_PSELSIZE 32 
/* cache replacement policy */
enum cache_policy {
  LRU,		/* replace least recently used block (perfect LRU) */
  Random,	/* replace a random block */
	BIP,	/* promoting the lines from LRU position to the MRU position only if they are used while in the LRU position*/
	LIP,	/* on any insertion if counter is 0 then insert on MRU, else on LRU */
	DIP,		/* Dynamic Insertion Policy */ 
	PLRU,		/* Tree-based Pseudo LRU */
  FIFO		/* replace the oldest block in the set */
};
/* type of set dueling for DIP */
enum DIP_SDM_type{
	LRU_set,	// LRU policy
	BIP_set,	// BIP policy		
	FOLLOWER_set	//policy decided by PSEL
};
