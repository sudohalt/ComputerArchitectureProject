/*opyright (C) 1994-2003 by Todd M. Austin, Ph.D. and SimpleScalar, LLC.
 * All Rights Reserved. 
 *  
		  ...
		  int argc, char **argv)        /* command line arguments */
{
  ...
  if (!mystricmp(cache_dl1_opt, "none"))
    ...
    {
      int sscanf_results = sscanf(cache_dl1_opt, "%[^:]:%d:%d:%d:%c",
		 name, &nsets, &bsize, &assoc, &c);
	  if (sscanf_results != 5 && sscanf_results != 6)
	fatal("OUTORDER bad l1 D-cache parms: <name>:<nsets>:<bsize>:<assoc>:<repl>:<width_BIPCTR>");
      cache_dl1 = cache_create(name, nsets, bsize, /* balloc */FALSE,
			       /* usize */0, assoc, cache_char2policy(c),
			       dl1_access_fn, /* hit lat */cache_dl1_lat, width_BIPCTR);
      ...
      else
	{
	  ...
	  if (sscanf_results2 != 5 && sscanf_results2 != 6)
	    fatal("bad l2 D-cache parms: "
		  "<name>:<nsets>:<bsize>:<assoc>:<repl>:<width_BIPCTR>");
	  cache_dl2 = cache_create(name, nsets, bsize, /* balloc */FALSE,
				   /* usize */0, assoc, cache_char2policy(c),
				   dl2_access_fn, /* hit lat */cache_dl2_lat, width_BIPCTR);
	}
    }
  ...
  if (!mystricmp(cache_il1_opt, "none"))
  ...
  else /* il1 is defined */
    {
      int sscanf_results3 = sscanf(cache_il1_opt, "%[^:]:%d:%d:%d:%c:%d",
		 name, &nsets, &bsize, &assoc, &c, &width_BIPCTR);
      if (sscanf_results3 != 5 && sscanf_results3 != 6)
	fatal("bad l1 I-cache parms: <name>:<nsets>:<bsize>:<assoc>:<repl>:<width_BIPCTR>");
      cache_il1 = cache_create(name, nsets, bsize, /* balloc */FALSE,
			       /* usize */0, assoc, cache_char2policy(c),
			       il1_access_fn, /* hit lat */cache_il1_lat, width_BIPCTR);
      ...
      else
	{
	    int sscanf_results4 = sscanf(cache_il2_opt, "%[^:]:%d:%d:%d:%c:%d",
		     name, &nsets, &bsize, &assoc, &c, &width_BIPCTR);
        if (sscanf_results4 != 5 && sscanf_results4 != 6)
	    fatal("bad l2 I-cache parms: "
		  "<name>:<nsets>:<bsize>:<assoc>:<repl>:<width_BIPCTR>");
	  cache_il2 = cache_create(name, nsets, bsize, /* balloc */FALSE,
				   /* usize */0, assoc, cache_char2policy(c),
				   il2_access_fn, /* hit lat */cache_il2_lat, width_BIPCTR);
	}
    }
  ...
  if (!mystricmp(itlb_opt, "none"))
  ...
  else
    {
      if (sscanf(itlb_opt, "%[^:]:%d:%d:%d:%c:%d",
		 name, &nsets, &bsize, &assoc, &c, &width_BIPCTR) != 5)
