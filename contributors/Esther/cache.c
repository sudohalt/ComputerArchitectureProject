...
*	Function
*	get_DIP_set_type
*	get the set type under DIP-SD
*/
enum DIP_SDM_type
get_DIP_set_type(int set_ID, int nsets) {
	     ...
	     int width_PSEL)
{
  ...
  for (bindex=0,i=0; i<nsets; i++)
    {
      if (cp->policy == DIP) {
      	get_DIP_set_type(i, nsets); // initialize set type when using DIP policy
      }
