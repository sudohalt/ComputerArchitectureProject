	     ...
	     unsigned int hit_latency)	/* latency in cycles for a hit */
{
  ...
  /* allocate the cache structure */
	...
	//add in size of policy A and B sets
	if(policy == GARP_LIP_LRU || policy == GARP_LIP_FIFO || policy ==  GARP_LIP_RAND || policy ==  GARP_LRU_RAND ||
 		policy ==  GARP_LRU_FIFO || policy ==   GARP_RAND_FIFO)
		cp = (struct cache_t *) calloc(1, sizeof(struct cache_t) + (nsets-1)*sizeof(struct cache_set_t) + 
										(nsets-1)*sizeof(struct cache_set_t)*2);
	else
		cp = (struct cache_t *) calloc(1, sizeof(struct cache_t) + (nsets-1)*sizeof(struct cache_set_t));
  if (!cp)
  ...
  /*set A and B policy*/ 
  if (cp->policy == GARP_LIP_LRU)
  {
  	cp->Apolicy = LIP;
  	cp->Bpolicy = LRU;
  }
  else if (cp->policy ==  GARP_LIP_FIFO)
  {
    cp->Apolicy = LIP;
  	cp->Bpolicy = FIFO;
  }
    else if (cp->policy ==  GARP_LIP_RAND)
  {
    cp->Apolicy = LIP;
  	cp->Bpolicy = Random;
  }
    else if (cp->policy ==  GARP_LRU_RAND)
  {
    cp->Apolicy = LRU;
  	cp->Bpolicy = Random;
  }
    else if (cp->policy ==  GARP_LRU_FIFO)
  {
    cp->Apolicy = LRU;
  	cp->Bpolicy = FIFO;
  }
  else if (cp->policy ==  GARP_RAND_FIFO)
  {
    cp->Apolicy = Random;
  	cp->Bpolicy = FIFO;
  }
  ...
  /* initialize counter for BIP Jon Added*/
  cp->BIP_counter = 0;
  ...
  if (!cp->data)
	...
	if(policy == GARP_LIP_LRU || policy == GARP_LIP_FIFO || policy ==  GARP_LIP_RAND || policy ==  GARP_LRU_RAND ||
 		policy ==  GARP_LRU_FIFO || policy ==   GARP_RAND_FIFO)
  {
    cp->policy_a_faults = 0;
  	cp->policy_b_faults = 0;
  	cp->policy_a_time = 0;  	
  	cp->policy_b_time = 0;
    cp->Adata = (byte_t *)calloc(nsets * assoc,
			      sizeof(struct cache_blk_t));
  ...
  if (!cp->Adata)
    ...
    cp->Bdata = (byte_t *)calloc(nsets * assoc,
      			sizeof(struct cache_blk_t));
  if (!cp->Adata)
    fatal("out of virtual memory");	 
  ...
  #endif  
  	/* recent faults bit array */
  	cp->recent_faults =(int  *)calloc(nsets * assoc,sizeof(int));
  	cp->recent_faults_index = 0;

      /* slice up the data blocks for A & B sets*/
  for (bindex=0,i=0; i<nsets; i++)
    {
      cp->Asets[i].way_head = NULL;
      cp->Asets[i].way_tail = NULL;
      
      cp->Bsets[i].way_head = NULL;
      cp->Bsets[i].way_tail = NULL;
	 ...
	 during random replacement selection) */
      cp->Asets[i].blks = CACHE_BINDEX(cp, cp->Adata, bindex);
      cp->Bsets[i].blks = CACHE_BINDEX(cp, cp->Bdata, bindex);
      ...
      for (j=0; j<assoc; j++)
	{
	  ...
	  blk = CACHE_BINDEX(cp, cp->Adata, bindex);	  
	  ...
	  blk->way_next = cp->Asets[i].way_head;
	  blk->way_prev = NULL;
	  if (cp->Asets[i].way_head)
	    cp->Asets[i].way_head->way_prev = blk;
	  cp->Asets[i].way_head = blk;
	  if (!cp->Asets[i].way_tail)
	    cp->Asets[i].way_tail = blk;
	  
	  
	  	  /* locate next cache block */
	  blk = CACHE_BINDEX(cp, cp->Bdata, bindex);	  
	  ...
	  /* insert into head of way list, order is arbitrary at this point */
	  blk->way_next = cp->Bsets[i].way_head;
	  blk->way_prev = NULL;
	  if (cp->Bsets[i].way_head)
	    cp->Bsets[i].way_head->way_prev = blk;
	  cp->Bsets[i].way_head = blk;
	  if (!cp->Bsets[i].way_tail)
	    cp->Bsets[i].way_tail = blk;  
  ...
  printf("cache: %s: %d-way, `%s' replacement policy, write-back\n",
	  ...
	  : cp->policy == FIFO ? "FIFO"
	  : cp->policy == BIP16 ? "BIP16" /*Jon - Added*/
	  : cp->policy == BIP32 ? "BIP32" /*Jon - Added*/
	  : cp->policy == BIP64 ? "BIP64" /*Jon - Added*/
	  : cp->policy == BIP128 ? "BIP128" /*Jon - Added*/
	  : cp->policy == LIP ? "LIP" /*Jon - Added*/
	  : cp->policy == GARP_LIP_LRU ? "GARP_LIP_LRU" /*Jon - Added*/
	  : cp->policy == GARP_LIP_FIFO ? "GARP_LIP_FIFO" /*Jon - Added*/
	  : cp->policy == GARP_LIP_RAND ? "GARP_LIP_RAND" /*Jon - Added*/
	  : cp->policy == GARP_LRU_RAND ? "GARP_LRU_RAND" /*Jon - Added*/
	  : cp->policy == GARP_LRU_FIFO ? "GARP_LRU_FIFO" /*Jon - Added*/
	  : cp->policy == GARP_RAND_FIFO ? "GARP_RAND_FIFO" /*Jon - Added*/
...
cache_char2policy(char c)		/* replacement policy as a char */
{
  ...
  case 'f': return FIFO;
  case 'b': return BIP16; /*Jon - Added*/
  case 'c': return BIP32; /*Jon - Added*/
  case 'd': return BIP64; /*Jon - Added*/
  case 'e': return BIP128; /*Jon - Added*/
  case 'g': return LIP; /*Jon - Added*/
  case 's': return GARP_LIP_LRU;/*Jon - Added*/
  case 't': return GARP_LIP_FIFO;
  case 'u': return GARP_LIP_RAND;
  case 'v': return GARP_LRU_RAND;
  case 'w': return GARP_LRU_FIFO;
  case 'x': return GARP_RAND_FIFO;
  ...
  stat_reg_formula(sdb, buf, "invalidation rate (i.e., invs/ref)", buf1, NULL);
  	if(cp->policy == GARP_LIP_LRU || cp->policy == GARP_LIP_FIFO || cp->policy ==  GARP_LIP_RAND || cp->policy ==  GARP_LRU_RAND ||
 		cp->policy ==  GARP_LRU_FIFO || cp->policy ==   GARP_RAND_FIFO)
 		{
 		  ...
 		  sprintf(buf, "%s.Policy_A_Misses", name);
  			stat_reg_counter(sdb, buf, "total number policy A of misses", &cp->policy_a_faults, 0, NULL);
  			
  			sprintf(buf, "%s.Policy_B_Misses", name);
  			stat_reg_counter(sdb, buf, "total number policy B of misses", &cp->policy_b_faults, 0, NULL);
  			  			
  			
  			 sprintf(buf, "%s.Policy_A_Time", name);
  			stat_reg_counter(sdb, buf, "total number of times Policy A was selected", &cp->policy_a_time, 0, NULL);
  			
   			 sprintf(buf, "%s.Policy_B_Time", name);
  			stat_reg_counter(sdb, buf, "total number of times Policy B was selected", &cp->policy_b_time, 0, NULL);	
  			
  			  sprintf(buf, "%s.Policy_A_Percentage", name);
  			 sprintf(buf1, "%s.Policy_A_Time / %s.misses", name, name);
  			stat_reg_formula(sdb, buf, "percentage of time policy A was selected (i.e., policyA/misses)", buf1, NULL);		
  			
  			 sprintf(buf, "%s.Policy_B_Percentage", name);
  			 sprintf(buf1, "%s.Policy_B_Time / %s.misses", name, name);
  			stat_reg_formula(sdb, buf, "percentage of time policy B was selected (i.e., policyB/misses)", buf1, NULL);	
  			//printf("Policy A Faults: %d \r\n",cp->policy_a_faults);
  			//printf("Policy B Faults: %d \r\n",cp->policy_b_faults);
  			//printf("Policy A-B Faults: %d \r\n",cp->policy_ab_faults);
  			//sprintf(buf, "%s.Policy A ", name);
  		//	cp->policy_a_time
 		}
}
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  struct cache_blk_t *blk_A, *blk_B, *repl_A, *repl_B;
  int lat = 0;
  int block_found_A, block_found_B,k,i,s;
  int bindex;

  #ifdef GARP_PRINT_DEBUG
  if (strcmp(cp->name,"dl1") == 0)
  {
	/*  printf("cache %s access: %s Bytes: %d addr 0x%08x Set: %d Tag: %x\r\n",
		  cp->name,  cmd == Write ? "Write"
		  : cmd == Read ? "Read"
		  : (abort(), ""),nbytes,addr,set,tag);*/
  }
  #endif
  ...
  /* permissions are checked on cache misses */
    ...
    block_found_B = 0; 
 		...
 		cp->policy ==  GARP_LRU_FIFO || cp->policy ==   GARP_RAND_FIFO)
 	{
      ...
      for (blk_A=cp->Asets[set].way_head; blk_A;  blk_A=blk_A->way_next)
		{
	  		if (blk_A->tag == tag ) // block found
	  		{
	    		block_found_A = 1;
	    		break;
	    	}
		}
		/* check for block in B */
      for (blk_B=cp->Bsets[set].way_head;  blk_B;  blk_B=blk_B->way_next)
		{
	  		if (blk_B->tag == tag) // block found
	  		{
	    		block_found_B = 1;
	    		break;
	    	}
		...
		if( !block_found_A  || !block_found_B)
		{
			...
			if(cp->recent_faults_index >= cp->nsets * cp->assoc)
				...
				cp->recent_faults_index++; 
		}
		if(!block_found_A) 
		{
	   		cp->policy_a_faults++;
	   		  #ifdef GARP_PRINT_DEBUG
			  	printf("Tag not found in A. Policy A Faults: %d\r\n",cp->policy_a_faults);
			  #endif
	   		switch(cp->Apolicy)
		   	{
		   		case LRU:
	  			case FIFO:
	  			    repl_A = cp->Asets[set].way_tail;
	   				update_way_list(&cp->Asets[set], repl_A, Head);
					break;
	  			case LIP:
	  			    repl_A = cp->Asets[set].way_tail;
					update_way_list(&cp->Asets[set], repl_A, Tail);
	   				break;
	  			case Random:
	  				bindex = myrand() & (cp->assoc - 1);
		  			repl_A = CACHE_BINDEX(cp, cp->Asets[set].blks, bindex);
					break;
				default:
		   	  ...
		   	  /* update block tags */
  			repl_A->tag = tag;
  			repl_A->status = CACHE_BLK_VALID;	/* dirty bit set on update */
	   	}
	   	else
	   	{
			   	  /* if LRU replacement and this is not the first element of list, reorder */
		  if (blk_A->way_prev && cp->Apolicy == LRU)
			{
			  /* move this block to head of the way (MRU) list */
			  update_way_list(&cp->Asets[set], blk_A, Head);
			}
    
		  /* if LIP/BIP16/BIP32/BIP64/BIP128 replacement and this is the last element of the list, reorder */
		  if (!blk_A->way_next && (cp->Apolicy == LIP || cp->Apolicy == BIP16 || cp->Apolicy == BIP32  
			  || cp->Apolicy == BIP64 || cp->Apolicy == BIP128))
			{
			#ifdef GARP_PRINT_DEBUG
			  //	printf("Update for BIP-LIP\r\n");
			  #endif
			  /* move this block to head of the way (MRU) list */
			  update_way_list(&cp->Asets[set], blk_A, Head);
			}
	   	...
	   	if(!block_found_B) 
		{
			  cp->policy_b_faults++;
			  #ifdef GARP_PRINT_DEBUG
			  	printf("Tag not found in B. Policy B Faults: %d\r\n",cp->policy_b_faults);
			  #endif
	   		...
	   		switch(cp->Bpolicy)
		   	{
		   		case LRU:
	  			case FIFO:
	  			    repl_B = cp->Bsets[set].way_tail;
	   				update_way_list(&cp->Bsets[set], repl_B, Head);
					break;
	  			case LIP:
	  			    repl_B = cp->Bsets[set].way_tail;
					update_way_list(&cp->Bsets[set], repl_B, Tail);
	   				break;
	  			case Random:
		  			bindex = myrand() & (cp->assoc - 1);
		  			repl_B = CACHE_BINDEX(cp, cp->Bsets[set].blks, bindex);
					break;
				default:
    				panic("bogus replacement policy");
		   	}
		   	  /* update block tags */
  			repl_B->tag = tag;
  			repl_B->status = CACHE_BLK_VALID;	/* dirty bit set on update */

	   	}
	   	else
	   	{
	   		  #ifdef GARP_PRINT_DEBUG
			 // 	printf("Tag found in B\r\n");
			  #endif
			   	  /* if LRU replacement and this is not the first element of list, reorder */
		  if (blk_B->way_prev && cp->Bpolicy == LRU)
			{
			  /* move this block to head of the way (MRU) list */
			  update_way_list(&cp->Bsets[set], blk_B, Head);
			}
    
		  /* if LIP/BIP16/BIP32/BIP64/BIP128 replacement and this is the last element of the list, reorder */
		  if (!blk_B->way_next && (cp->Bpolicy == LIP || cp->Bpolicy == BIP16 || cp->Bpolicy == BIP32  
			  || cp->Bpolicy == BIP64 || cp->Bpolicy == BIP128))
			{
			  /* move this block to head of the way (MRU) list */
			  update_way_list(&cp->Bsets[set], blk_B, Head);
			}
 ...
 cache_hit: /* slow hit handler */
  ...
  /* if LIP/BIP16/BIP32/BIP64/BIP128 replacement and this is the last element of the list, reorder */
  if (!blk->way_next && (cp->policy == LIP || cp->policy == BIP16 || cp->policy == BIP32  
      || cp->policy == BIP64 || cp->policy == BIP128))
    {
      /* move this block to head of the way (MRU) list */
      update_way_list(&cp->sets[set], blk, Head);
    }
