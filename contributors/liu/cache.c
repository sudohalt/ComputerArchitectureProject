...
/* get the set type */
enum DIP_SDM_type get_DIP_set_type  (int setid, int nsets){
	enum DIP_SDM_type DIP_set_type;
	if ( setid % (nsets/32) == 0 ) DIP_set_type = LRU_set; 		 	
	else if ( (setid+1) % (nsets/32) == 0 ) DIP_set_type = BIP_set; 	 	
	else	DIP_set_type = FOLLOWER_set; 	 		 	
	return DIP_set_type; 	 
}
...
//========================================================================================================///
		...
		cp->width_BIPCTR = DEFAULT_BIPCTRSIZE;
		cp->width_PSEL = DEFAULT_PSELSIZE;
		cp->BIPCTR = 0;
		cp->PSEL = 0;
//========================================================================================================///
  ...
  for (bindex=0,i=0; i<nsets; i++)
    {
	/* for DIP, init set type*/
	if(cp->policy == DIP) get_DIP_set_type(i, nsets);
...
/* create and initialize a general cache structure */
struct cache_t *			/* pointer to cache created */
cache_create2(char *name,		/* name of the cache */
	     int nsets,			/* total number of sets in cache */
	     int bsize,			/* block (line) size of cache */
	     int balloc,		/* allocate data space for blocks? */
	     int usize,			/* size of user data to alloc w/blks */
...
// obtain the block index when using Pseudo-PLRU policy
int
...
// update Pseudo LRU state according to the ind of accessed blk
int
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  case Random:
	...
	case BIP:
    		repl = cp->sets[set].way_tail;
		if ( cp->BIPCTR == 0 ) update_way_list(&cp->sets[set], repl, Head);
		else update_way_list(&cp->sets[set], repl, Tail);
//DEFAULT_BIPCTRSIZE 
		if ( cp->BIPCTR < DEFAULT_BIPCTRSIZE ) 	cp->BIPCTR ++;
		else cp->BIPCTR = 0;
    		break;
	...
	case DIP:
		...
		if ( cp->sets[set].DIP_set_type == LRU_set ) {
			where = Head;
			if ( cp->PSEL < DEFAULT_PSELSIZE ) 	cp->PSEL ++;
		}
		else if ( cp->sets[set].DIP_set_type == BIP_set ) {
			if ( cp->BIPCTR == 0 ) where = Head;
			else where = Tail;
			if ( cp->PSEL > 0 ) cp->PSEL --;
		}
		else {
 ...
 cache_hit: /* slow hit handler */
  ...
  if (blk->way_prev && cp->policy == LRU)
	...
	if (cp->policy == PLRU)
	{
			/* find the index of the block in its set */
			int bindex = 0;
  ...
  if (blk->way_prev && ((cp->policy == LIP) ||(cp->policy == DIP)))
    {
      /* move this block to head of the way (MRU) list */
      update_way_list(&cp->sets[set], blk, Head);
    }
