...
"    <repl>   - block replacement strategy, 'l'-LRU, 'f'-FIFO, 'r'-random, 'b'-BIP\n"
"\n"
...
{
  char name[128], c;
  int nsets, bsize, assoc, width_BIPCTR;
  ...
  if (!mystricmp(cache_dl1_opt, "none"))
    ...
    {
	  int sscanf_results = sscanf(cache_dl1_opt, "%[^:]:%d:%d:%d:%c",name, &nsets, &bsize, &assoc, &c);
	if (sscanf_results != 6 && sscanf_results != 5)
	fatal("SIM_CACHE bad l1 D-cache parms: <name>:<nsets>:<bsize>:<assoc>:<repl>:<width_BIPCTR>");
      cache_dl1 = cache_create(name, nsets, bsize, /* balloc */FALSE,
			       /* usize */0, assoc, cache_char2policy(c),
			       dl1_access_fn, width_BIPCTR,/* hit latency */1);
	...
	{
	  int sscanf_results2 = sscanf(cache_dl2_opt, "%[^:]:%d:%d:%d:%c:%d",
		     name, &nsets, &bsize, &assoc, &c, &width_BIPCTR);
      if (sscanf_results2 != 6 && sscanf_results2 != 5)
	    fatal("bad l2 D-cache parms: "
		  "<name>:<nsets>:<bsize>:<assoc>:<repl>:<width_BIPCTR>");
	  cache_dl2 = cache_create(name, nsets, bsize, /* balloc */FALSE,
				   /* usize */0, assoc, cache_char2policy(c),
				   dl2_access_fn, width_BIPCTR, /* hit latency */1);
	}
    }
  ...
  if (!mystricmp(cache_il1_opt, "none"))
    ...
    {
      int sscanf_results3 = sscanf(cache_il1_opt, "%[^:]:%d:%d:%d:%c:%d",
		 name, &nsets, &bsize, &assoc, &c, &width_BIPCTR);
	  if (sscanf_results3 != 6 && sscanf_results3 != 5)
	fatal("bad l1 I-cache parms: <name>:<nsets>:<bsize>:<assoc>:<repl>");
      cache_il1 = cache_create(name, nsets, bsize, /* balloc */FALSE,
			       /* usize */0, assoc, cache_char2policy(c),
			       il1_access_fn, width_BIPCTR, /* hit latency */1);
	...
	{
	  int sscanf_results4 = sscanf(cache_il2_opt, "%[^:]:%d:%d:%d:%c:%d",
		     name, &nsets, &bsize, &assoc, &c, &width_BIPCTR);
	  if (sscanf_results4 != 6 && sscanf_results4 != 5) 
	    fatal("bad l2 I-cache parms: "
		  "<name>:<nsets>:<bsize>:<assoc>:<repl>:<width_BIPCTR>");
	  cache_il2 = cache_create(name, nsets, bsize, /* balloc */FALSE,
				   /* usize */0, assoc, cache_char2policy(c),
				   il2_access_fn, width_BIPCTR, /* hit latency */1);
	}
    }
  ...
  if (!mystricmp(itlb_opt, "none"))
    ...
    {
      int sscanf_results5 = sscanf(itlb_opt, "%[^:]:%d:%d:%d:%c:%d",
		 name, &nsets, &bsize, &assoc, &c, &width_BIPCTR); 
     if (sscanf_results5 != 5 && sscanf_results5 != 6)
	fatal("bad TLB parms: <name>:<nsets>:<page_size>:<assoc>:<repl>:<width_BIPCTR>");
      itlb = cache_create(name, nsets, bsize, /* balloc */FALSE,
			  /* usize */sizeof(md_addr_t), assoc,
			  cache_char2policy(c), itlb_access_fn, width_BIPCTR,
			  /* hit latency */1);
    }
  ...
  if (!mystricmp(dtlb_opt, "none"))
    ...
    {
      int sscanf_results6 = sscanf(dtlb_opt, "%[^:]:%d:%d:%d:%c:%d",
		 name, &nsets, &bsize, &assoc, &c, &width_BIPCTR);
       if (sscanf_results6 != 5 && sscanf_results6 != 6)
	fatal("bad TLB parms: <name>:<nsets>:<page_size>:<assoc>:<repl>:<width_BIPCTR>");
      dtlb = cache_create(name, nsets, bsize, /* balloc */FALSE,
			  /* usize */sizeof(md_addr_t), assoc,
			  cache_char2policy(c), dtlb_access_fn, width_BIPCTR, 
			  /* hit latency */1);
    }
}
