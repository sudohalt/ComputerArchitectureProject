...
cache_create(char *name,		/* name of the cache */
	     ...
	     unsigned int (*blk_access_fn)(enum mem_cmd cmd,
					   ...
					   tick_t now),
	     unsigned int hit_latency, int width_BIPCTR)	/* latency in cycles for a hit */
	     ...
	     md_addr_t *repl_addr)	/* for address of replaced block */
{
  ...
  case LIP:
   {
     // Insert at head instead of tail (lru position)
      repl = cp->sets[set].way_head;
      // When accessed while in that position then move it to the mru position
      update_way_list(&cp->sets[set], repl, Head);
      break;
   }
  case BIP:
	{
      ...
      int max_BIPCTR = cp->width_BIPCTR-1;
